    ...
    private void initHandlerMapping() {
        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            if(aClass.isAnnotationPresent(Security.class)) {
                ...
                userNames = annotation.value(); // 等同于 {'张三','李四'}
                flag = false;
            }
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                //如果contoller被权限控制则不再需要判断方法
                if(flag&&method.isAnnotationPresent(Security.class)) {
                    Security security = method.getAnnotation(Security.class);
                    userNames = security.value(); // 等同于 {'张三','李四'}
                }
...
//        method.invoke() //
        ...
        String[] userNames = handler.getUserName();
        if(userNames!=null&&userNames.length>0){
            String value =  StringUtils.join(req.getParameterMap().get("userName"), ",");
            if(!Arrays.asList(userNames).contains(value)){
                resp.getWriter().write("403 user forbidden");
                return;
            }
        }
